  ...
  <section class="clan-page page-content">
    ...
    <template v-if="clan">
      <section class="clan-banner">
        ...
        <aside><strong>{{ clan.memberCount }}</strong><span>MEMBERS</span><small><i></i>{{ clan.online }} online</small></aside>
      </section>
      ...
      <section class="clan-card finder-card">
        ...
        <div class="public-clans">
          <article v-for="entry in filteredClans" :key="entry.id">
            <b>🛡</b><div><strong>{{ entry.name }}</strong><p>{{ entry.description || 'Open to new adventurers.' }}</p><small>{{ entry.memberCount }} member{{ entry.memberCount === 1 ? '' : 's' }} · Creator {{ entry.ownerName }}</small></div><button class="accent" type="button" :disabled="busy" @click="emit('join', entry.id)">JOIN</button>
          </article>
