...
<template>
  ...
  <main v-else class="game-shell" :class="{ 'high-scores-open': page === 'high scores', 'auction-open': page === 'auction', 'factions-open': page === 'factions' }">
    ...
    <section v-else-if="page === 'woodcutting' || page === 'mining'" class="page-content">
      <div class="page-heading"><div><p class="eyebrow">GATHERING · LEVEL {{ professions[page].level }}</p><h1>{{ page }}</h1><p>Levels mainly improve speed. A quick harvest can finish sooner, but always gives the same yield.</p></div><div class="skill-xp">SKILL XP {{ professions[page].xp }} / {{ professionXpNeeded(page) }}</div></div>
      <div class="profession-stats"><div><span>Speed</span><strong>+{{ professionStats(page).speed.toFixed(1) }}%</strong><small>Every action takes less time</small></div><div><span>Yield</span><strong>{{ professionStats(page).yield }}</strong><small>Guaranteed items per action · upgrades are rare</small></div><div><span>Quick chance</span><strong>{{ professionStats(page).critChance.toFixed(1) }}%</strong><small>Chance for a faster action</small></div><div><span>Quick speed</span><strong>{{ professionStats(page).critPower.toFixed(2) }}×</strong><small>How much faster it finishes</small></div></div>
