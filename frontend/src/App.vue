...
<template>
  ...
  <main v-else class="game-shell" :class="{ 'high-scores-open': page === 'high scores', 'auction-open': page === 'auction', 'factions-open': page === 'factions' }">
    ...
    <ClanPage v-else-if="page === 'clans'" :clan="clan" :invitations="clanInvitations" :public-clans="publicClans" :inventory="inventory" :player-name="playerName" :combat-stats="combatStats" :raid-combat="clanRaidCombat" :error="clanError" :notice="clanNotice" :busy="clanActionRunning" :gift-error="giftError" :gift-notice="giftNotice" :gift-busy="giftRunning" @refresh="loadClans" @create="createClan" @join="joinClan" @invite="inviteClanMember" @accept="acceptClanInvitation" @decline="declineClanInvitation" @leave="leaveClan" @disband="disbandClan" @contribute="contributeToClan" @fight-raid="fightClanRaid" @gift="sendGift" />
