...
<template>
  ...
  <main v-else class="game-shell" :class="{ 'high-scores-open': page === 'high scores', 'auction-open': page === 'auction', 'factions-open': page === 'factions' }">
    ...
    <section v-else-if="page === 'woodcutting' || page === 'mining' || page === 'fishing' || page === 'farming'" class="page-content">
      ...
      <article v-if="page === 'fishing' && equipment.fishingRod" class="fishing-tool-guide"><div class="fishing-tool-icon">{{ gearCatalog[equipment.fishingRod]?.icon }}</div><div><span>EQUIPPED FISHING TOOL</span><h2>{{ gearCatalog[equipment.fishingRod]?.name }}</h2><p>{{ gearCatalog[equipment.fishingRod]?.description }}</p><small>{{ Object.entries(gearCatalog[equipment.fishingRod]?.bonuses || {}).map(([stat,value]) => formatBonus(stat, Number(value))).join(' · ') || 'Starter rod · upgrade it to improve fishing stats' }}</small></div><aside><button type="button" @click="page = 'shop'">UPGRADE IN SHOP</button><button type="button" @click="page = 'crafting'">CRAFT A ROD</button></aside></article>
      <article v-if="page === 'farming' && equipment.farmingHoe" class="fishing-tool-guide farming-tool-guide"><div class="fishing-tool-icon">{{ gearCatalog[equipment.farmingHoe]?.icon }}</div><div><span>EQUIPPED FARMING TOOL</span><h2>{{ gearCatalog[equipment.farmingHoe]?.name }}</h2><p>{{ gearCatalog[equipment.farmingHoe]?.description }}</p><small>{{ Object.entries(gearCatalog[equipment.farmingHoe]?.bonuses || {}).map(([stat,value]) => formatBonus(stat, Number(value))).join(' · ') || 'Starter hoe · upgrade it to improve farming stats' }}</small></div><aside><button type="button" @click="page = 'shop'">UPGRADE IN SHOP</button><button type="button" @click="page = 'crafting'">CRAFT A HOE</button></aside></article>
      <div class="resource-grid"><article v-for="resource in page === 'woodcutting' ? woods : page === 'mining' ? rocks : page === 'fishing' ? fishingSpots : farmingPlots" :key="resource.id" class="resource-card" :class="{ locked: !isUnlocked(resource) }" :style="{ '--accent': resource.color }" :title="resourceTooltip(resource)"><div class="resource-icon">{{ resource.icon }}</div><div><span class="tier">TIER {{ resource.tier }} · {{ resource.family }}</span><h3>{{ resource.name }}</h3><p>{{ resource.item }} · {{ effectiveDuration(resource).toFixed(1) }}s effective</p><small>Mastery {{ resourceMastery[resource.id] || 0 }} · +{{ Math.floor((resourceMastery[resource.id] || 0) / 10) }}% speed</small></div><div class="action-area"><div class="meter" :class="{ critical: jobs[page]?.id === resource.id && jobs[page]?.critical }"><i :style="{ width: `${jobs[page]?.id === resource.id ? jobs[page]?.progress : 0}%` }"></i></div><button @click="gather(resource)" :disabled="!!jobs[page] || !isUnlocked(resource)">{{ !isUnlocked(resource) ? `SKILL LV ${resource.tier}` : jobs[page]?.id === resource.id ? `${jobs[page]?.critical ? 'CRIT · ' : ''}${Math.floor(jobs[page]?.progress || 0)}%` : page === 'woodcutting' ? 'CHOP' : page === 'mining' ? 'MINE' : page === 'fishing' ? 'CAST LINE' : 'HARVEST' }}</button></div><b class="owned">{{ inventory[resource.item] || 0 }} owned</b></article></div>
    </section>
  ...
  <section v-if="playerName && page === 'high scores'" class="leaderboard leaderboard-page">
    ...
    <div class="leaderboard-tabs">
      ...
      <button :class="{ selected: leaderboardCategory === 'fishing' }" @click="loadLeaderboard('fishing')">
        Fishing
      </button>
