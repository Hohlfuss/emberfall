...
<template>
  ...
  <main v-else class="game-shell" :class="{ 'high-scores-open': page === 'high scores', 'auction-open': page === 'auction', 'factions-open': page === 'factions' }">
    ...
    <section v-else-if="page === 'woodcutting' || page === 'mining'" class="page-content">
      <div class="page-heading"><div><p class="eyebrow">GATHERING · LEVEL {{ professions[page].level }}</p><h1>{{ page }}</h1><p>Levels mainly improve speed. Critical harvests and bonus yield roll independently.</p></div><div class="skill-xp">SKILL XP {{ professions[page].xp }} / {{ professionXpNeeded(page) }}</div></div>
