...
import CookingInventoryStats from './CookingInventoryStats.vue'
...
const {
  ...
  woods, rocks, fishingSpots, farmingPlots, allResources, rareMaterials, gearCatalog, slotLabels, gearSlots, shopUpgradeDetails, professions, jobs, inventory, sellPrices,
...
<template>
  ...
  <main v-else class="game-shell" :class="{ 'high-scores-open': page === 'high scores', 'auction-open': page === 'auction', 'factions-open': page === 'factions' }">
    ...
    <CookingPage v-else-if="page === 'cooking'" :recipes="cookingRecipeList" :inventory="inventory" :cooking-id="cookingId" :profession="cookingProfession" :stats="cookingStats" :healing-values="foodHealingValues" :hot-values="foodHotValues" @cook="cook" />
  ...
  <Teleport v-if="playerName && page === 'inventory'" defer to=".inventory-column:last-child">
    ...
    <CookingInventoryStats :profession="cookingProfession" :stats="cookingStats" />
  </Teleport>
