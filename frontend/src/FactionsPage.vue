...
import type { FactionDefinition, FactionId } from './useGame'
  ...
  <section class="factions-page">
    ...
    <div class="faction-grid">
      <article v-for="faction in definitions" :key="faction.id" :class="{ allied: allied === faction.id, locked: level < faction.unlockLevel }">
        <b>{{ faction.icon }}</b><span class="tier">{{ allied === faction.id ? 'CURRENT ALLIANCE' : level < faction.unlockLevel ? `UNLOCKS AT LEVEL ${faction.unlockLevel}` : 'AVAILABLE' }}</span><h2>{{ faction.name }}</h2><p>{{ faction.description }}</p>
