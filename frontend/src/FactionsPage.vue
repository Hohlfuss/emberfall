  ...
  <section class="factions-page">
    ...
    <div class="faction-grid">
      <article v-for="faction in definitions" :key="faction.id" :class="{ allied: allied === faction.id, locked: level < faction.unlockLevel }">
        ...
        <ol><li v-for="(reward,index) in faction.rewards" :key="reward" :class="{ earned: (progress[faction.id]?.rank || 0) > index }"><span>Rank {{ index + 1 }}</span>{{ reward }} <small>+{{ (index + 1) * 100 }} gold{{ index === 3 ? ' · +1 worker' : '' }}</small></li></ol>
