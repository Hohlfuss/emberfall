...
type Food = { item: string; name: string; icon: string; healing: number; owned: number }
type CombatStats = { maxHealth: number; attack: number; defense: number; attackSpeed: number; recoveryTime: number }
    ...
    <div class="combatants">
      <article class="combatant hero-card">
        ...
        <section class="food-dock">
          <div><label for="battle-food">FOOD</label><select id="battle-food" :value="selectedFood || ''" @change="selectFood"><option value="">Select food</option><option v-for="food in foods" :key="food.item" :value="food.item">{{ food.icon }} {{ food.name }} · {{ food.owned }} owned · +{{ food.healing }} HP</option></select></div>
          <button type="button" :disabled="!selectedFood || !selectedFoodDetails()?.owned || health >= combatStats.maxHealth || recovering" @click="selectedFood && emit('eat', selectedFood)">EAT <span v-if="selectedFoodDetails()">+{{ selectedFoodDetails()?.healing }} HP</span></button>
        ...
        <div class="combat-stats"><div><span>Attack</span><strong>{{ combatStats.attack }}</strong></div><div><span>Defense</span><strong>{{ combatStats.defense }}</strong></div><div><span>Speed</span><strong>{{ (combatStats.attackSpeed / 1000).toFixed(2) }}s</strong></div></div>
      </article>
      ...
      <article class="combatant enemy-card" :class="{ loading: enemyLoading }">
        ...
        <template v-else>
          ...
          <div class="combat-stats"><div><span>Attack</span><strong>{{ enemy.attack }}</strong></div><div><span>Defense</span><strong>{{ enemy.defense }}</strong></div><div><span>Speed</span><strong>{{ (enemy.attackSpeed / 1000).toFixed(2) }}s</strong></div></div>
