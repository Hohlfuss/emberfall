...
type CombatStats = { maxHealth: number; attack: number; defense: number; attackSpeed: number }
...
type FloatingDamage = { id: number; target: 'hero' | 'enemy'; amount: number; lane: number }
const floatingDamage = ref<FloatingDamage[]>([])
const damageTimers = new Map<number, ReturnType<typeof setTimeout>>()
let nextDamageId = 1
const heroStruck = computed(() => floatingDamage.value.some(hit => hit.target === 'hero'))
const enemyStruck = computed(() => floatingDamage.value.some(hit => hit.target === 'enemy'))

function showDamage(target: FloatingDamage['target'], amount: number) {
  if (!Number.isFinite(amount) || amount <= 0) return
  const id = nextDamageId++
  floatingDamage.value.push({ id, target, amount: Math.round(amount), lane: id % 3 })
  damageTimers.set(id, setTimeout(() => {
    floatingDamage.value = floatingDamage.value.filter(hit => hit.id !== id)
    damageTimers.delete(id)
  }, 1_150))
}

watch(() => props.health, (health, previous) => {
  if (health < previous) showDamage('hero', previous - health)
})

watch(() => [props.enemy.name, props.enemy.health] as const, ([name, health], [previousName, previousHealth]) => {
  if (name === previousName && health < previousHealth) showDamage('enemy', previousHealth - health)
})

onBeforeUnmount(() => damageTimers.forEach(timer => clearTimeout(timer)))
...
<template>
  <section class="battle-page" :class="{ active: battleStarted, 'boss-mode': encounterMode === 'boss' }">
    ...
    <div class="combatants">
      <article class="combatant hero-card" :class="{ struck: heroStruck }">
        <div class="damage-stream hero-damage" aria-hidden="true"><b v-for="hit in floatingDamage.filter(item => item.target === 'hero')" :key="hit.id" :class="`lane-${hit.lane}`">−{{ hit.amount }}</b></div>
        ...
        <div class="combat-stats" aria-label="Player combat stats"><div><span>Attack</span><strong>{{ combatStats.attack }}</strong></div><div><span>Defense</span><strong>{{ combatStats.defense }}</strong></div><div><span>Interval</span><strong>{{ (combatStats.attackSpeed / 1000).toFixed(2) }}s</strong></div></div>
      ...
      <article class="combatant enemy-card" :class="{ loading: enemyLoading, struck: enemyStruck }">
        <div class="damage-stream enemy-damage" aria-hidden="true"><b v-for="hit in floatingDamage.filter(item => item.target === 'enemy')" :key="hit.id" :class="`lane-${hit.lane}`">−{{ hit.amount }}</b></div>
        ...
        <template v-else>
          ...
          <div class="combat-stats enemy-combat-stats" aria-label="Enemy combat stats"><div><span>Attack</span><strong>{{ enemy.attack }}</strong></div><div><span>Defense</span><strong>{{ enemy.defense }}</strong></div><div><span>Interval</span><strong>{{ (enemy.attackSpeed / 1000).toFixed(2) }}s</strong></div></div>
